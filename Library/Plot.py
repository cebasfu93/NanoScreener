...
def plot_all_angs(cath):
    ...
    fig, axs = plt.subplots(
        figsize=(28, 28),
        ncols=len(AA_ordered),
        nrows=len(AA_ordered),
        sharex=True,
        sharey=True,
        subplot_kw={
            "xlim": (0, 5),
    ...
    plt.savefig("{}/Plots/{}/all_angles.png".format(cath.setup, cath.rname), format="png")
    plt.show()
    plt.close()
...
def plot_all_forces(cath):
    ...
    for ax in axs.flatten():
        ax.tick_params(labelsize=Z)
    plt.savefig("{}/Plots/{}/all_forces.png".format(cath.setup, cath.rname), format="png")
    plt.show()
    plt.close()
...
def plot_all_dgs(cath):
    ...
    for ax in axs.flatten():
        ax.tick_params(labelsize=Z)
    plt.savefig("{}/Plots/{}/all_gibbs.png".format(cath.setup, cath.rname), format="png")
    plt.show()
    plt.close()
...
def plot_dg_distributions(cath):
    ...
    for ax in axs.flatten():
        ax.tick_params(labelsize=Z)
    plt.savefig("{}/Plots/{}/all_dg_distr.png".format(cath.setup, cath.rname), format="png")
    plt.show()
    plt.close()


def plot_dg_map(
    cath, show_mean=True, show_std=False, svg=False, THRESH=0, dg_min=3, dg_max=18
):
    """
    cath is a Catecholamine_PMF object (see Preprocessing.py)
    ...
    for a1, aa1 in enumerate(AA_ordered):
        for a2, aa2 in enumerate(AA_ordered):
            ...
            pic_downlim[a1, a2] = -1 * (
                cath.g_mean.get(key, np.nan) - cath.g_std.get(key, np.nan)
            )
    ...
    ax.set_xticks(
        np.linspace(-0.5, len(AA_ordered) - 0.5, len(AA_ordered) + 1), minor=True
    )
    ...
    if show_mean:
        for a1, aa1 in enumerate(AA_ordered):
            for a2, aa2 in enumerate(AA_ordered):
                if (
                    pic[a1, a2] > THRESH - 1.0
                ):  # THRESHOLD. 1.0 is expected deviation from PMF methods
                    label = "{:.1f}".format(pic[a1, a2])
                    if show_std:
                        label += "\n{:.1f}".format(
                            cath.g_std["{}-{}".format(aa1, aa2).upper()]
                        )
                    ax.text(
                        a2,
                        a1,
    ...
    if svg:
        plt.savefig(
            "{}/Plots/{}/all_dg_map.png".format(cath.setup, cath.rname),
            format="png",
            bbox_inches="tight",
            dpi=300,
        )
        plt.savefig(
            "{}/Plots/{}/all_dg_map.svg".format(cath.setup, cath.rname),
            format="svg",
            bbox_inches="tight",
        )
    else:
...
def plot_aa_dependence(cath):
    ...
    mp = dict(ls="-", lw=1.0, color="k")
    cp = dict(ls="-", lw=1.5, color="k")
    wp = dict(ls="-", lw=1.5, color="k")

    fig, axs = plt.subplots(
        figsize=(7, 5), ncols=1, nrows=2, gridspec_kw={"hspace": 0.4}
    )
    ...
    for ax, g_col, xlab in zip(axs, grouping_col, xlabels):
        ...
        ax.grid()
    plt.savefig(
        "{}/Plots/{}/aa_dependence.svg".format(cath.setup, cath.rname),
        format="svg",
        bbox_inches="tight",
    )
    plt.savefig(
        "{}/Plots/{}/aa_dependence.png".format(cath.setup, cath.rname),
        format="png",
        dpi=300,
        bbox_inches="tight",
    )
