...
class Input:
    def __init__(self, sim, metric, mode):
        ...
        if "31" in sim or "32" in sim or "33" in sim:
            ...
            self.kwargs_rogs = {'xlim':(0,10), 'xticks':(0,5), 'ylim':(0.8,1.7), 'yticks':(0.8,1.1,1.4)}
            self.kwargs_sasas = {'xlim':(0,10), 'xticks':(0,5), 'ylim':(90, 330), 'yticks':(90,170,250)}
        elif "34" in sim:
            self.endtime = 6
            self.dist_suffix = "NVT2"
            self.kwargs_rogs = {'xlim':(0,6), 'xticks':(0,2,4), 'ylim':(0.9,1.3), 'yticks':(0.9,1.1)}
            self.kwargs_sasas = {'xlim':(0,6), 'xticks':(0,2,4), 'ylim':(80, 160), 'yticks':(80, 120)}
        ...
        if metric == 'Bye_time':
            ...
            'ylabel': r"$\eta$ factor", 'yticks': np.linspace(0, len(self.i_space), 6)[:-1], 'yticklabels': np.round(np.linspace(min(self.i_space), max(self.i_space), 6),1),
            ...
            self.kwargs_paramsearch = {'xlabel': 'Bound fraction', 'xticks': np.linspace(0, len(self.j_space), 6)[:-1], 'xticklabels': np.round(np.linspace(min(self.j_space), max(self.j_space), 6),2)[:-1],
          'ylabel': r"$\eta$ factor", 'yticks': np.linspace(0, len(self.i_space), 6)[:-1], 'yticklabels': np.round(np.linspace(min(self.i_space), max(self.i_space), 6),1),
          'path': self.plot_path, 'plot_str': self.plot_str}
          ...
          'path': self.plot_path, 'plot_str': self.plot_str}
            ...
            self.j_space = np.linspace(0, 1, 100)
            self.kwargs_paramsearch = {'xlabel': 'Weighted fraction', 'xticks': np.linspace(0, len(self.j_space), 6)[:-1], 'xticklabels': np.round(np.linspace(min(self.j_space), max(self.j_space), 6),2)[:-1],
          'ylabel': r"$\eta$ factor", 'yticks': np.linspace(0, len(self.i_space), 6)[:-1], 'yticklabels': np.round(np.linspace(min(self.i_space), max(self.i_space), 6),1),
          'path': self.plot_path, 'plot_str': self.plot_str}
