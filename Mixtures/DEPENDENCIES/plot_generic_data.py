...
def plot_all_rog(times, data=DATA, nw=50, kwargs={}):
    ...
    fig, axs = plt.subplots(figsize=(2*ncol, 2*nrow), ncols=ncol, nrows=nrow, sharex=True, sharey=True, subplot_kw={'ylim':kwargs.get('ylim',(0.9,1.3)), 'xlim':kwargs.get('xlim',(0,10)), 'xticks':kwargs.get('xticks',[0,5])}, gridspec_kw={'hspace':0.0, 'wspace':0.0})
...
def plot_rog_byact(data=DATA):
    ...
    axs.bar(data.index, data['Rog_Mean'], yerr=data['Rog_Std'], color=data.Color, linewidth=2, ec='k', width=0.7, capsize=3)
    plt.show()
    plt.close()
...
def plot_rog_bynp(data=DATA):
    ...
    axs.set_ylabel("R.O.G. (nm)", fontsize=Z)
    axs.bar(df_sorted.index, df_sorted['Rog_Mean'], yerr=df_sorted['Rog_Std'], color=df_sorted.Color, linewidth=2, ec='k', width=0.7, capsize=3)
    plt.show()
    plt.close()
...
def plot_all_sasa(times, data=DATA, nw=50, kwargs={}):
    ...
    fig, axs = plt.subplots(figsize=(2*ncol, 2*nrow), ncols=ncol, nrows=nrow, sharex=True, sharey=True, subplot_kw={'ylim':kwargs.get('ylim',(80,160)), 'xlim':kwargs.get('xlim',(0,10)), 'xticks':kwargs.get('xticks',[0,5])}, gridspec_kw={'hspace':0.0, 'wspace':0.0})
    for n, nn in enumerate(np.sort(data.Nanoparticle.unique())[::-1]):
...
def plot_sasa_byact(data=DATA):
    ...
    axs.bar(data.index, data['Sasa_Mean'], yerr=data['Sasa_Std'], color=data.Color, linewidth=2, ec='k', width=0.7, capsize=3)
    plt.show()
    plt.close()
...
def plot_sasa_bynp(data=DATA):
    ...
    axs.set_xlabel("Nanoparticle", fontsize=Z)
    axs.set_ylabel("S.A.S.A. ($nm^2$)", fontsize=Z)
    axs.bar(df_sorted.index, df_sorted['Sasa_Mean'], yerr=df_sorted['Sasa_Std'], color=df_sorted.Color, linewidth=2, ec='k', width=0.7, capsize=3)
    plt.show()
    plt.close()
