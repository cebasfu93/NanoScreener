...
# DEPRECATED but maybe useful for future machine learning
...
def plot_my_tree(tree, ml_cols):
    ...
    ax = plt.axes()
    plot_tree(tree, ax=ax, filled=True, feature_names=ml_cols, class_names=[
              'Inactive', 'Active'], rounded=True, rotate=True, fontsize=Z - 4)
    plt.show()
    plt.close()
