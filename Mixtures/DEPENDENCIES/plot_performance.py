...
def plot_distributions(j_space, best_j, bind_scores, unbind_scores, kwargs={}):
    ...
    """
    fig, axs = plt.subplots(figsize=kwargs.get('figsize', (11, 8)),
                            ncols=1, nrows=2, gridspec_kw={'hspace': 0.35})
    ...
    axs[1].plot(np.append(bins[:-1], [axs[1].get_xlim()[1]]),
                np.append(np.cumsum(n * (bins[1] - bins[0])), [1]), color='b', lw=3.5)
    ...
    axs[1].plot(np.append(bins[:-1], [axs[1].get_xlim()[1]]),
                np.append(np.cumsum(n * (bins[1] - bins[0])), [1]), color='orange', lw=3.5)
    ...
    if kwargs.get('svg', False):
        plt.savefig(kwargs.get('path', '.') + "/Distributions_" +
                    kwargs.get('plot_str', 'def') + ".svg", format='svg', bbox_inches='tight')
    else:
        plt.savefig(kwargs.get('path', '.') + "/Distributions_" + kwargs.get('plot_str', 'def') + ".png",
...
def plot_score_distribution(best_j, j_space, data=DATA, kwargs={}):
    ...
    fig, axs = plt.subplots(figsize=kwargs.get('figsize', (11, 8)), sharex=True,
                            ncols=1, nrows=3, gridspec_kw={'hspace': 0.2, 'height_ratios': [3, 1, 0.25]})
    for ax in axs:
    ...
    for act_log, c, score in zip(data.Act_log, data.Color, data.Score):
        ...
        axs[0].errorbar(np.mean(score), act_log, xerr=np.std(score) / (len(score)**0.5), c=c, fmt='o',
                        ms=7, mec='k', mew=1.5, zorder=50, elinewidth=0.6, ecolor='k', capsize=2)  # boltz_mean
        axs[1].plot(smooth_j, proc.gaussian(smooth_j, np.mean(score), np.std(
            score) / (len(score))**0.5), color=c, alpha=0.7, lw=2.5)  # ste
        axs[2].axvline(x=np.mean(score), ymin=0, ymax=1, color=c, lw=2.5)

    if kwargs.get('svg', False):
        print(kwargs.get('path', '.') + "/scoredistr_" + kwargs.get('plot_str', 'def') + ".svg")
        plt.savefig(kwargs.get('path', '.') + "/scoredistr_" + kwargs.get('plot_str', 'def') + ".svg",
                    format='svg', bbox_inches='tight', transparent=True)
    else:
        plt.savefig(kwargs.get('path', '.') + "/scoredistr_" + kwargs.get('plot_str', 'def') + ".png",
                    format='png', dpi=300, bbox_inches='tight', transparent=True)
    plt.show()
    plt.close()
...
def plot_score_rank(best_j, j_space, data=DATA, kwargs={}):
    ...
    ax.axvline(x=best_j, ymin=0, ymax=1, c='k', lw=2, ls='-', zorder=100)
    ax.axhline(y=data.Act_Rank[np.where(~data.Active)
                               [0][-1]] - 0.5, c='k', lw=2, ls='-', zorder=100)
    for i, (score, rank, c) in enumerate(zip(data.Score, data.Act_Rank, data.Color)):
        ax.errorbar([score], [rank], c=c, fmt='o', ms=5, alpha=0.5)
        ax.errorbar(np.mean(score), [rank], xerr=np.std(score), c=c, fmt='o', ms=7,
                    mec='k', mew=1.5, zorder=50, elinewidth=1.3, ecolor='k', capsize=2)
    plt.savefig(kwargs.get('path', '.') + "/scoredistr_rank_" + kwargs.get('plot_str', 'def') + ".png",
...
def plot_np_dependence(best_j, data=DATA, kwargs={}):
    ...
    """
    fig, axs = plt.subplots(figsize=(20, 26), ncols=3, nrows=4,
    ...
    for a, (ax, NN) in enumerate(zip(axs.flatten(), np.unique(data.Nanoparticle))):
        ...
        for i, (score, act_log, c, nn) in enumerate(zip(data.Score, data.Act_log, data.Color, data.Nanoparticle)):
            ...
            ax.errorbar(np.mean(score), [act_log], xerr=np.std(
                score), c=c, fmt='o', ms=7, mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    for ax in axs[:, 0]:
    ...
    axs[-1, -1].set_yticks([])
    plt.savefig(kwargs.get('path', '.') + "/np_dependence_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_np_dependence_rank(best_j, data=DATA, kwargs={}):
    ...
    highlighting the dyads with a certain nanoparticle
    """
    fig, axs = plt.subplots(figsize=(20, 32), ncols=3, nrows=4,
                            gridspec_kw={'hspace': 0.1, 'wspace': 0.03})
    for a, (ax, NN) in enumerate(zip(axs.flatten(), np.unique(data.Nanoparticle))):
        ax2 = ax.twinx()
        ax2.set_ylim(31, -1)
        ...
        ax.set_yticks([])
        ax.axvline(x=best_j, ymin=0, ymax=1, c='k', lw=0.9, ls='-', zorder=100)
        ax.axhline(y=data.Act_Rank[np.where(~data.Active)[0][-1]] - 0.5,
                   c='k', lw=0.9, ls='-', zorder=100)
        ax.tick_params(labelsize=Z)
        ax.set_xlim(kwargs.get('xlim', (0, 1)))
        for i, (score, rank, c, nn) in enumerate(zip(data.Score, data.Act_Rank, data.Color, data.Nanoparticle)):
            ...
            ax.errorbar(np.mean(score), [rank], xerr=np.std(
                score), c=c, fmt='o', ms=7, mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    for ax in axs[:, 0]:
    ...
    ax2.grid(alpha=0.2)
    ax2.set_yticklabels(data.index)
    plt.savefig(kwargs.get('path', '.') + "/np_dependence_rank_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_an_dependence(best_j, data=DATA, kwargs={}):
    ...
    """
    fig, axs = plt.subplots(figsize=(16, 39), ncols=3, nrows=6,
                            gridspec_kw={'hspace': 0.1, 'wspace': 0.03})
    ...
    axs[-1, -2].set_yticks([])
    axs[-1, -1].set_yticks([])
    plt.savefig(kwargs.get('path', '.') + "/an_dependence_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_an_dependence_rank(best_j, data=DATA, kwargs={}):
    ...
    highlighting the dyads with a certain analyte
    """
    fig, axs = plt.subplots(figsize=(20, 48), ncols=3, nrows=6,
                            gridspec_kw={'hspace': 0.1, 'wspace': 0.03})
    for a, (ax, AA) in enumerate(zip(axs.flatten(), np.unique(data.Analyte))):
        ax2 = ax.twinx()
        ax2.set_ylim(31, -1)
    ...
    ax2.grid(alpha=0.2)
    ax2.set_yticklabels(data.index)
    plt.savefig(kwargs.get('path', '.') + "/an_dependence_rank_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_npcharge_dependence(best_j, data=DATA, kwargs={}):
    ...
    fig, axs = plt.subplots(figsize=(10, 6), ncols=2, nrows=1,
                            gridspec_kw={'hspace': 0.1, 'wspace': 0.03})
    ...
    for i, (score, act_log, c, nn) in enumerate(zip(data.Score, data.Act_log, data.Color, data.Nanoparticle)):
        ...
        axs[0].errorbar(np.mean(score), [act_log], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    for i, (score, act_log, c, nn) in enumerate(zip(data.Score, data.Act_log, data.Color, data.Nanoparticle)):
        ...
        axs[1].errorbar(np.mean(score), [act_log], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)

    axs[0].set_yticks(np.linspace(0, 6, 7))
    axs[0].set_ylabel("Log(Activity)", fontsize=Z)
    plt.savefig(kwargs.get('path', '.') + "/npcharge_dependence_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_npcharge_dependence_rank(best_j, data=DATA, kwargs={}):
    ...
    for i, (score, rank, c, nn) in enumerate(zip(data.Score, data.Act_Rank, data.Color, data.Nanoparticle)):
        ...
        axs[0].errorbar(np.mean(score), [rank], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    for i, (score, rank, c, nn) in enumerate(zip(data.Score, data.Act_Rank, data.Color, data.Nanoparticle)):
        if nn not in charged_nps:
            al1 = 0.6
        ...
        axs[1].errorbar(np.mean(score), [rank], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    axs[0].set_yticks(np.linspace(0, 30, 7))
    axs[0].set_ylabel("Experimental ranking", fontsize=Z)
    plt.savefig(kwargs.get('path', '.') + "/npcharge_dependence_rank_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_ancharge_dependence(best_j, data=DATA, kwargs={}):
    ...
    for i, (score, act_log, c, aa) in enumerate(zip(data.Score, data.Act_log, data.Color, data.Analyte)):
        ...
        axs[2].errorbar(np.mean(score), [act_log], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)

    axs[0].set_yticks(np.linspace(0, 6, 7))
    axs[0].set_ylabel("Log(Activity)", fontsize=Z)
    plt.savefig(kwargs.get('path', '.') + "/ancharge_dependence_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
...
def plot_ancharge_dependence_rank(best_j, data=DATA, kwargs={}):
    ...
    """
    titles = ["Cationic As", "Anionic As", "Zwitterionic As"]
    fig, axs = plt.subplots(figsize=(18, 8), ncols=3, nrows=1,
                            gridspec_kw={'hspace': 0.1, 'wspace': 0.03})
    for ax, title in zip(axs, titles):
        ax2 = ax.twinx()
        ax2.set_ylim(31, -1)
    ...
    for i, (score, rank, c, aa) in enumerate(zip(data.Score, data.Act_Rank, data.Color, data.Analyte)):
        ...
        axs[2].errorbar(np.mean(score), [rank], xerr=np.std(score), c=c, fmt='o', ms=7,
                        mec='k', mew=1.5, zorder=50, elinewidth=0.8, ecolor='k', capsize=2, alpha=al2)
    axs[0].set_yticks(np.linspace(0, 30, 7))
    axs[0].set_ylabel("Experimental ranking", fontsize=Z)
    plt.savefig(kwargs.get('path', '.') + "/ancharge_dependence_rank_" + kwargs.get('plot_str', 'def') + ".png",
                format='png', dpi=300, bbox_inches='tight')
    plt.show()
    plt.close()
